...
export default function Header({ title, onMenuClick }: HeaderProps) {
    const { user } = useAuth();
    return (
        <header className="h-20 px-4 sm:px-8 flex items-center justify-between border-b border-slate-100 bg-white/80 backdrop-blur-md sticky top-0 z-40 gap-4">
            <div className="flex items-center space-x-3 sm:space-x-6 min-w-0">
                <button
                    onClick={onMenuClick}
                    className="lg:hidden text-slate-400 hover:text-slate-900 transition-colors shrink-0"
                >
                    <Menu className="h-5 w-5" />
                </button>
                <h2 className="text-sm font-bold tracking-tight text-slate-900 truncate">{title}</h2>
            </div>
            <div className="flex items-center space-x-4 sm:space-x-10 shrink-0">
